...
#Convert decimals to and from DB useable strings
def adapt_decimal(d):
...
class BotDatabase:
    ...
    #For each table, create ability to save, update, read, and delete elems in them. 
    #Will also be able to update/delete elems that are related, like all pairs
    def saveBot(self, bot):
        '''Add a bot profile to database
    ...
    def updateOrder(self, order):
        #Update bot details from database
        conn = sqlite3.connect(self.name, detect_types=sqlite3.PARSE_DECLTYPES)
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
    ...
    def getOpenOrdersOfBot(self, bot):
        ''' Get all bots within the DB '''
        conn = sqlite3.connect(self.name, detect_types=sqlite3.PARSE_DECLTYPES)
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        c.execute('SELECT * FROM orders WHERE bot_id = ? and closing_order_id = 0 and is_closed = 0', (bot['id'],))
